...
subhead: Базовый подход к созданию компонента вкладок, аналогичного используемым в приложениях для iOS и Android.
authors:
  - adamargyle
description: Базовый подход к созданию компонента вкладок, аналогичного используемым в приложениях для iOS и Android.
date: 2021-02-17
...
который будет адаптивным, совместимым с различными устройствами и браузерами.
Попробуйте [демонстрацию](https://gui-challenges.web.app/tabs/dist/).
...
<figure class="w-figure w-figure--fullbleed">
  ...
  <figcaption class="w-figure">
    <a href="https://gui-challenges.web.app/tabs/dist/">Демонстрация</a>
  </figcaption>
</figure>
...
Сегодня основной элемент взаимодействия с вкладками — область навигации по кнопкам,
который переключает видимость контенте в отображаемом фрейме. Различные
...
<figure class="w-figure">
  {% Img
    ...
    alt="Коллаж довольно хаотичный из-за огромного разнообразия стилей, которые применяются в Интернете к концепции этого компонента",
    class="w-screenshot",
...
вложенной страницы, а затем показываем обновление области контента, пока браузер прокручивает до
...
(здесь подходит `<section>`). Каждому хешу ссылки будет соответствовать раздел,
поэтому браузер сможет делать прокрутку посредством анкера.
...
Например, Chrome 89 при нажатии на ссылку фокус автоматически переключается на статью `:target` —
и не нужно ничего писать на JS. Пользователь может прокрутить содержимое статьи обычным образом
...
Затем я заполнил статьи различным количеством рыбы, ссылки —
заголовками различной длины с изображениями для заголовков. Контент у нас есть — можно приступать к работе
над макетом.

### Макет с прокруткой {: #overscroll }

В этом компоненте есть области прокрутки трех типов:
- Элемент навигации <b style="color: #FF00E2;">(розовый цвет)</b> использует горизонтальную
  прокрутку.
- Область контента <b style="color: #008CFF;">(синий цвет)</b> также использует горизонтальную
  прокрутку.
- Элементы статей <b style="color: #2FD800;">(зеленый цвет) используют вертикальную
  прокрутку.
...
1. **Безразмерная поверхность.** <br>В этом макете это контейнеры списка: ссылки
   навигации, статьи разделов и содержимое статей.

#### Макет `<snap-tabs>` {: #tabs-layout }

В качестве макета верхнего уровня я выбрал `flex` (адаптируемый блок) с направлением
`column` — чтобы заголовок и раздел располагались вертикально. Это первое
окно прокрутки; оно скрывает всё с помощью `overflow: hidden`. Заголовок и
и разделе затем будут использовать прокрутку за границы в виде отдельных зон.
...
snap-tabs {
  ...
  /* установить первичный контейнер */
  overflow: hidden;
  position: relative;
...
#### Макет `<header>` для вкладок {: #tabs-header }
...
такой макет заголовка позволяет этого добиться. Ни одного элемента с абсолютным размещением!
...
Далее — стили прокрутки. Оказывается, один стиль можно использовать
в двух областях горизонтальной прокрутки (заголовок и раздел), поэтому я сделал вспомогательный
класс: `.scroll-snap-x`.
...
#### Макет `<nav>` заголовков вкладок {: #tabs-header-nav }
...
Стили и размеры ссылок задаются автоматически, поэтому в макете `nav` нужно указать только
...
<figure class="w-figure">
  {% Img
    ...
    alt="Элементы «nav» помечены ярко-розовыми ярлыками и выделены рамкой, указывающей место, которое они занимают в компоненте, и направление развертывания",
    class="w-screenshot",
    width="800", height="327"
  %}
</figure>

#### Макет `<section>` для вкладок {: #tabs-section }
...
линия, без переноса (как нам и нужно), что позволяет заполнить родительское окно с выходом за его границы.

<figure class="w-figure">
  {% Img
    src="image/vS06HQ1YTsbMKSFTIPl2iogUQP73/FYroCMocutCGg1X8kfdG.png",
    alt="Элементы «article» помечены ярко-розовыми ярлыками и выделены рамкой указывающей место, которое они занимают в компоненте, и направление развертывания",
    class="w-screenshot",
...
#### Макет `<article>` для вкладок {: #tabs-article }

У пользователя должна быть возможность прокручивать содержимое статьи, причем полосы прокрутки должны
...
Браузер обрабатывает разнообразные взаимодействия с сенсорным экраном, мышью и клавиатурой,
...
что элементы `link` навигации ссылки и элементы `article` встраиваются и привязываются к началу
...
<figure class="w-figure">
  {% Img
    ...
    alt="Элемент «article» и его дочерние элементы помечены ярко-розовыми ярлыками и выделены рамкой указывающей место, которое они занимают в компоненте, и направление развертывания",
    class="w-screenshot",
...
это позволяет мне проверить макет и оркестровку прокрутки.
...
друг с другом. Каждый из этих фреймов окна прокрутки также является родительским элементом `flex`
...
<figure class="w-figure">
  ...
  %}
  <figcaption class="w-figure">
    Chromium DevTools: макет элемент `nav` адаптируемого блока, содержащий  элементы привязки,
    макет `section` с `grid` с элементами `article` и элементы `article`
    с абзацами и элементом заголовка.
  </figcaption>
</figure>

Макеты прокрутки готовы: с привязкой, ссылками на контент и возможностью использования
...
браузера. Попробуйте [Режим
устройства](https://developers.google.com/web/tools/chrome-devtools/device-mode) (Device Mode) в Chromium DevTools:
выберите любой режим, кроме **отзывчивого** (Responsive), а затем измените размер фрейма устройства.
...
интерфейса. Так мы поможет пользователю с удобством (надеюсь)
просмотреть весь контент. Я добавлю движение с определенной целью и
и условиями. Пользователи могут задавать [предпочтения
...
Это позволяет элементу `<nav>`, который действует
...
местах нам придется проявить особую внимательность!
...
Переключая это предпочтение в Chromium DevTools, я демонстрирую эти два
стиля переходов, во время работы над которыми я получил массу удовольствия.
...
В разделе выше я показал вам, как я работаю со стилями плавного перехода,
когда нужно ограничить движение. В этом разделе я покажу, как связать индикатор и
...
только экспериментальные реализации. Однако для нее есть полифил, который и я использую в этой
демонстрации.
...
делать переход между шириной со значением `auto`, динамически создавать
...
положение индикатора меняется от раздела к другому, в точности
...
Для каждой ссылки `nav` на вкладке нужна новая анимация цвета по той же
временно́й шкале, что и индикатор подчеркивания. Я использую ту же временну́ю шкалу, что и раньше:
...
Пользователи не всегда будут нажимать на вкладки или использовать клавиатуру — иногда они просто
использовать прокрутку. Когда компонент прокрутки в `section` останавливается,
...
Каждый при прокрутке элементов `section` мы очищаем время ожидания (если оно есть)
и начинаем новый отсчет. Когда прокрутка элемента `section` останавливается, мы не очищаем время ожидания,
а запускаем отсчет 100 мс после бездействия. По окончании отсчета вызываем функцию, которая определяет,
...
находится, я делаю его активным.
...
Иногда из них получается очаровательная парочка.
