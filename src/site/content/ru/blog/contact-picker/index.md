---
title: Средство выбора контактов для веб-страниц
...
Например, в почтовом веб-клиенте Contact Picker API позволит
...
| 3. Собрать отзывы и уточнить дизайн     | [Готово][spec]             |
...
При вызове этого метода он возвращает промис и показывает окно выбора контактов, в котором
пользователь может выбрать контакты, которые нужно  передать сайту. После
выбора контактов и нажатия на кнопку **Готово** промис разрешается и дает
массив соответствующих контактов.

При вызове `select()` в качестве первого параметра нужно указать массив свойств,
которые должны быть возвращены (допустимые значения:
`'name'`, `'email'`, `'tel'`, `'address'` или `'icon'`).
...
Помните, что не все свойства всегда доступны и могут добавляться
новые. В будущем другие платформы и источники контактов могут определять,
...
{% Aside 'caution' %}
  Ярлыки и другая семантическая информация в полях контактов отбрасывается.
{% endAside %}
...
включая пользовательский контроль, прозрачность и удобство. Разъяснение каждого из этих принципов — ниже.
...
<figure class="w-figure w-figure--inline-right">
  ...
  <figcaption class="w-figcaption">
    ...
    они не будут переданы, хотя сайт и запросил их.
  </figcaption>
</figure>

Выбрать все контакты сразу нельзя — это поощряет пользователей
