...
WebSocketStream API решает проблему отсутствия надлежащей обратной реакции
...
| 2. Создать исходный проект спецификации   | [В процессе][spec]          |
| 3. Собрать отзывы и уточнить дизайн     | [В процессе](#feedback)     |
...
Для начала мы создаем новый объект `WebSocketStream` передаем ему URL-адрес сервера WebSocket.
...
Аналогичным образом, вызывая метод
...
Авторы реализации WebSocketStream API — [Адам Райс](https://github.com/ricea) и
