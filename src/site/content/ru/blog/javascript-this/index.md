...
Стрелочные функции хороши тем, что внутреннее значение `this` не изменяется: оно _всегда_ такое же,
как у внешнего `this`.
...
Может показаться, что описанное выше нарушает правило «значение `this` всегда такое же, как у `this` в области действия родительского элемента»,
...
функции в конструкторе. Если по какой-то причине использовать поля класса, удобной альтернативой будет
...
При каждом вызове `boundFunction` значением ее `this` будет объект, переданный в `bind`
(`boundObject`).
...
Плохая репутация у `this` именно из-за использования такого подхода. Передайте нужное значение в качестве аргумента:
...
объектом: обычно такое поведение не ожидается. Плохая репутация у `this` именно из-за использования такого подхода. Передайте нужное
...
именно из-за использования такого подхода. Передайте нужное значение в качестве аргумента: это более явное поведение, и оно совместимо со
...
В этом случае значение `this` не определено. Выражение `'use strict'` в функции не требуется, если
...
технически, это не всегда так. Подробнее смотрите у [Матиаса
Байненса](https://mathiasbynens.be/notes/globalthis#terminology) (он также объясняет, почему
`globalThis`, а не просто `global`). {% endAside %}
